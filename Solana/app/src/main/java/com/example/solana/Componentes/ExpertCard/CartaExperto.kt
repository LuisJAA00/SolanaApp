...
)
{
    ...
    {
        Image(painter = painterResource(id = foto), contentDescription = null, modifier = Modifier.padding(4.dp))
        Column () {
            TextWithBottomLine(texto = Nombre,modifier = Modifier.padding(top = 4.dp))
            Text(text = info)
        }
    }
}
